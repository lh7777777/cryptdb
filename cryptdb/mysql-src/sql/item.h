...
//  	originChar[i] = '\0';
  ...
  char* EncodeVarbinary(){
	  char* Ptr = str_value.c_ptr_quick(); // 获得Ptr
//	  std::cout << "Ptr: " << Ptr << std::endl;
	  int len = (long)str_value.length();
	  char* encodePtr = EncodeBase64(Ptr, len);
//	  std::cout << "encodePtr: " << encodePtr << std::endl;
	  str_value.EncodePtr((char*)(encodePtr), len * 2); // 编码
	  char* decodePtr = DecodeBase64(str_value.c_ptr_quick(), len * 2);
//	  str_value.EncodePtr((char*)(decodePtr)); // 解码
	  return decodePtr;
  }
